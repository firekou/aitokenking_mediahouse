---
name: reel-dy87ypfiy49
description: （草稿）我最近发现了一些C-Lens的全新玩法 來源 IG Reel DY87ypFiY49。 逐字稿提到的工具（⚠️ 未經畫面確認）：Lens。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我最近发现了一些C-Lens的全新玩法 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DY87ypFiY49/](https://www.instagram.com/p/DY87ypFiY49/)｜2026-05-30｜
> 59 秒｜891 views
> **分流：** 動作分 6（門檻 3）｜訊號 ['序列詞', '先…再', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 我最近发现了一些C-Lens的全新玩法,以前很没有任何人讲过,一个视频全部教给你,真的超简单。

## 疑似步驟（規則萃取，未經整理）

1. 首先第一个,最近我们有讲过用C-Lens的全能参考去复刻别人的运镜,那如果你知道这个,我完全可以去创造自己脑子里面的画面,我们可以在Blender里面随意搭建一个简单的模型,然后再把你想要做的画面录下来,这样再难的运镜AI都可以给你复刻出来。
2. 如果你有自己的歌曲,想剪一段视频做MV,你可以直接把音频丢给C-Lens,AI会自动帮我们做口型同步,还有一个省额度的玩法,我不会一条一条镜头去生成,而是直接在提示词底下时间轴,一次生成就能生出完整曲线。
3. 很多人会在前期生成图片的时候加特效,但是这样会让你特效非常不稳定,我们可以先生成一张不带特效的画面,再把你想要的特效融合进去,最后用C-Lens生成,这样就会稳定很多,更多进阶的C-Lens玩法我都整理好了文档,有效的朋友可以找我拿,对于这三个玩法,你学会了吗?

## 逐字稿提到的工具

| 名稱 | 證據強度 | 說明 |
|---|---|---|
| `Lens` | ⚠️ **E5** | 逐字稿聽寫，**未經畫面確認**。實測顯示同一工具可能有多種錯拼 |
