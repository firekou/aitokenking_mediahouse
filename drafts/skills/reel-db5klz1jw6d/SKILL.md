---
name: reel-db5klz1jw6d
description: （草稿）你是不是也在为视频的配音头疼? 來源 IG Reel Db5kLZ1jW6d。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 你是不是也在为视频的配音头疼? — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/Db5kLZ1jW6d/](https://www.instagram.com/p/Db5kLZ1jW6d/)｜2026-08-11｜
> 106 秒｜3991 views
> **分流：** 動作分 6（門檻 3）｜訊號 ['序列詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
1. 首先一条视频的声音设计主要分为两个板块,一个是背景音乐,另一个是画面音效。
2. 但现在有了AI整个过程都会简单很多,我们只需要把做好视频丢给CodeX,让它分析每个镜头的内容,画面的剪辑节奏以及整条视频的情绪变化,然后让它生成一份音乐提示词,然后再把这段提示词交给SeedMusic,就能得到一首根据画面节奏专门生成的背景音乐。
3. 在这里,我们同样可以使用CodeX,首先把视频交给它,让它提取关键针并分析每个时间点发生的动作,接着生成一张完整的音效时间表。
4. 这里要注意一定要让它写清楚这几个点,然后让CodeX为每个时间节点的声音生成对应的提示词,最后再交给COD5生成就好。
5. 这里一定要注意不要选错模型了,Stick模型是做音乐的,COD5是做配音的,最后再把这些放在一起,整个视频的音效就完成啦。
