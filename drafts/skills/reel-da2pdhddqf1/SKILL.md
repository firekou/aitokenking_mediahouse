---
name: reel-da2pdhddqf1
description: （草稿）如何用AI把你的画面从这样变成这样呢? 來源 IG Reel Da2pDHdDqF1。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
metadata:
  ...
  mediahouse-source-shortcode: "Da2pDHdDqF1"
...
# 如何用AI把你的画面从这样变成这样呢? — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/Da2pDHdDqF1/](https://www.instagram.com/p/Da2pDHdDqF1/)｜2026-07-16｜
> 100 秒｜11716 views
> **分流：** 動作分 11（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞', '操作動詞', '教學祈使', '具體參數']
> **語言：** 一律繁體中文輸出。
...
1. 这,是我最近发现的一个AI新玩法,不论是TVC广告还是品牌短片,它都能让你的画面更高级,那今天我就从零开始教你这个全新玩法,老规矩,资料我都准备好了,有需要的朋友可以自取,首先不管你做的是短剧,TVC广告还是品牌短片,目前AI单条最多只能生成15秒,所以一支完整作品肯定要拆成好几段来生成,那应该怎么让这些分段画面从一开始就保持统一呢?
2. 答案很简单,就是在生成之前先定一套色彩基准,具体该怎么做呢?
3. 第一步呢,就是先去找参考,不过这里我不建议大家手动去找内容,除非你有很大的阅片量,我们可以把剧本上传给AI,让它分析片子适合什么样的色彩,并且帮你找一些参考,最后再把这些参考图传给AI,让它提取出这张图的色卡,包括整体调性,颜色名字以及使用范围,用色卡会比直接给参考图更稳定,那在生图阶段其实还有一个关键,就是给AI一套固定的摄影参数,大家可以直接把我的这个格式传给AI,让它根据参考图帮你反推全职摄影的提示词,有了统一的基础画面以后,再把这些图拿去生视频,画面就会更稳定,但这里有一个特别重要的点,AI指出的视频不能直接当成最终成片,因为视频模型在生成动作的时候会重新计算光线,尤其是多段视频拼起来以后,人物肤色,环境色还是会有细微变化,所以最关键的一步一定是最后的手动调色,教大家一个偷懒的技巧,我们可以直接把视频和参考一起传给Codec,让它帮你操作检验调色,这里最重要的就是要给它加一个色散的效果,这样画面会多一点真实镜头的呼吸感,大家可以看看调色前后的对比,这次用到的色卡模板,摄影提示词,还有Codec的调色指令,我都整理好了,有需要的朋友可以自取,用这个方法几乎所有的画面都能搞定,所以你学会了吗?
