---
name: reel-dy3lfbtoadl
description: （草稿）看到的这些AI画面都是用我的Skill生成的 來源 IG Reel DY3lfbTOAdL。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 看到的这些AI画面都是用我的Skill生成的 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DY3lfbTOAdL/](https://www.instagram.com/p/DY3lfbTOAdL/)｜2026-05-28｜
> 132 秒｜8404 views
> **分流：** 動作分 7（門檻 3）｜訊號 ['序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 你现在看到的这些AI画面都是用我的Skill生成的,并且中间没写任何提示词。

## 疑似步驟（規則萃取，未經整理）

1. 你现在看到的这些AI画面都是用我的Skill生成的,并且中间没写任何提示词。
2. 首先我们随便打开一个AI工具,然后先告诉AI自己的需求。
3. 比如我这个Skill,我先告诉AI我的需求是让大家能够生成像我这样的画面,然后AI就会自动拆解流程,并且告诉我需要准备什么样的素材。
4. 我这里上传了一些我往日的一些作品,和常用的生图和生视频的提示词。
5. 因为你说的越具体AI模仿你的准确度才会越高,接着让AI帮你生成一个这样的文档,你只需要下载文档并上传给AI就能调用你的Skill生图,我们来看看它的效果。
6. 很多人做到这一步就停了,但是这样其实是不够的,因为真正能使用的Skill一定是需要不断调整的,我们可以把Skill上传之后使用,如果发现哪里流程不对,就回头告诉AI修改的点,比如我这个Skill,我发现AI给我的视频提示词之后,它就不能继续生成下一张图,于是我直接把这个问题告诉AI,让它帮我修改逻辑,现在的AI模型是有能力帮你修改Skill的,修改完之后它就能一直连续生成,那我的这个Skill应该怎么用呢?
7. 首先我们上传AI之后,它会引导你选择想要的结果,比如人物外观,人物性格和情绪,接着拿着这一份提示词去banana或者急忙的生图,再把这张图丢回给AI,它就能自动分析图片的状态,生成一段视频提示词,整个流程不需要你自己写任何东西,你会发现以前我们是在操作AI,现在更像是在训练一个懂你的创作助手,当然如果你有好的Skill也可以分享出来,也可以调用我的Skill去生成,所以这个看起来超难的Skill,你学会了吗?
