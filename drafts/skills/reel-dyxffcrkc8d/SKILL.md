---
name: reel-dyxffcrkc8d
description: （草稿）我发现很多人对氛围感有很大的误区 來源 IG Reel DYxFFcRkc8D。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我发现很多人对氛围感有很大的误区 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYxFFcRkc8D/](https://www.instagram.com/p/DYxFFcRkc8D/)｜2026-05-25｜
> 106 秒｜667 views
> **分流：** 動作分 5（門檻 3）｜訊號 ['步驟序數（第N步）', '先…再']
> **語言：** 一律繁體中文輸出。
...
> 我发现很多人对氛围感有很大的误区,比如这个画面,我希望它更温暖一点,很多人会直接让AI给画面加温暖氛围,但这是完全错误的,因为一张图的氛围感其实是由三个层面组成的,明暗、冷暖、虚实。

## 疑似步驟（規則萃取，未經整理）

1. 所以,第一步你必须先告诉AI哪里是高光,哪里是暗部,当亮暗拉开情绪才有落点。
2. 正确做法是先确定一个主情绪颜色,再把其他颜色全部降级,不管你要的是温暖、孤独还是高级感,逻辑都一样,当颜色有主次,画面自然就有情绪。
