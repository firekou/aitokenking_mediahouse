---
name: reel-dbnmxzadybu
description: （草稿）我们学如何用集梦做这种电商产品模特图 來源 IG Reel DbnMXzaDYBU。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我们学如何用集梦做这种电商产品模特图 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DbnMXzaDYBU/](https://www.instagram.com/p/DbnMXzaDYBU/)｜2026-08-04｜
> 59 秒｜1364 views
...
> 今天我们学如何用集梦做这种电商产品模特图。

## 疑似步驟（規則萃取，未經整理）

1. 首先我们先打开豆包上传我们想要生成的效果图来反推我们的关键词。
2. 我们让豆包描述一下这张图片然后整理成关键词,这样就可以得到图片的关键词了。
3. 接着我们打开集梦黏贴提示词之后就可以等待生成结果了。
4. 不过这个环节需要多试几次才能生成出精致的效果。
5. 然后我们打开Libribe找到这个工作流,点击右边的Confirmit就会来到这个界面。
6. 我们在左边上传我们的产品图,在右边上传我们的模特图,然后在你模特手里的产品画上蒙版就可以点击生成了。
7. 当然你也可以选择运行应用,然后在左侧上传你的产品和模特图同样也是能达到这样的效果的哦。
