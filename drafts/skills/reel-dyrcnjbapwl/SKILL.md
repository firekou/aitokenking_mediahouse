---
name: reel-dyrcnjbapwl
description: （草稿）我发现很多人提取风格就是把图丢给AI让它反推提示词 來源 IG Reel DYrcNJBAPWl。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我发现很多人提取风格就是把图丢给AI让它反推提示词 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYrcNJBAPWl/](https://www.instagram.com/p/DYrcNJBAPWl/)｜2026-05-23｜
> 89 秒｜262 views
...
> 我发现很多人提取风格就是把图丢给AI让它反推提示词，这样提取到的根本不是风格而是那次生成的结果。

## 疑似步驟（規則萃取，未經整理）

1. 我发现很多人提取风格就是把图丢给AI让它反推提示词，这样提取到的根本不是风格而是那次生成的结果。
2. 第一步不要看画面有什么而是看没什么。
3. 而这些词对生成几乎没有任何约束力。
4. 正确的问法应该是问AI这张图在生成时有哪些不可变的条件。
5. 第二步让AI看共性而不是看独立。
6. 这些被反复保留下来的选择才是真正的风格。
7. 第三步复用画面结构搭建体系。
8. 真正有价值的是把前两步得到的条件翻译成AI能长期执行的生成结构。
9. 所以风格从来不是提示词而是一套被反复执行的选择逻辑。
