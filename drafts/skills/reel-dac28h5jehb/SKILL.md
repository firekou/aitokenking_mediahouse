---
name: reel-dac28h5jehb
description: （草稿）看到的这些AI画面 來源 IG Reel Dac28H5jeHB。 逐字稿提到的工具（⚠️ 未經畫面確認）：Fable。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/Dac28H5jeHB/](https://www.instagram.com/p/Dac28H5jeHB/)｜2026-07-06｜
> 120 秒｜6144 views
> **分流：** 動作分 4（門檻 3）｜訊號 ['序列詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 你现在看到的这些AI画面,我全程没写一句提示词,全是Cloud帮我做的,还有这个枪战游戏以及这些特效,都是通过Cloud最新的Fable 5模型生成的。

## 疑似步驟（規則萃取，未經整理）

1. 你现在看到的这些AI画面,我全程没写一句提示词,全是Cloud帮我做的,还有这个枪战游戏以及这些特效,都是通过Cloud最新的Fable 5模型生成的。
2. 首先,Fable 5其实就是一个编程大模型,正是因为它恐怖的编程能力,所以衍生出了非常多的玩法,那这里我就先讲三个最有意思的,带大家一步步了解它到底该怎么玩。
3. 首先第一个,用Fable 5做游戏,这是目前最适合展示它能力的玩法,小到贪吃蛇,大到给它爱,它都能做出来。
4. 比如我这个射击类的FPS游戏,我就只告诉它了这句话,它就自己开始写代码来打造游戏,包括角色设定,任务模式,还有世界观,它都能自动给你生成,你只需要不断试玩,然后告诉它哪里不满意,让它修改就可以了。
5. 还有这种二维小游戏就更别说了,同一句提示词,这是它和Codex生成的对比,大家可以看看。
6. 但Fable 5厉害的地方是,它不只是会写代码,它还懂设计,比如用Fable 5做网页,这个玩法也特别适合小白,我们只需要找一个参考页面丢给Cloud,然后再给它一些你的素材,它就能直接帮你生成一个可以互动的网页,就连这种特效它都能给你生成。
7. 我这里有一个比较省Token的玩法,就是先用我这个基础词给我出个提示词,然后再把这段提示词传给Fable 5让它帮我们优化,告诉它你的需求,这样不管是人物的动作还是镜头的运动,生成的效果会好非常多,不过这里也要提醒大家,Fable 5确实很强,但它真的不便宜,我测试的时候只是随便说了一句你好,就扣了差不多两块钱人民币,所以千万不要把它当普通聊天工具用,那真的太浪费了。

## 逐字稿提到的工具

| 名稱 | 證據強度 | 說明 |
|---|---|---|
| `Fable` | ⚠️ **E5** | 逐字稿聽寫，**未經畫面確認**。實測顯示同一工具可能有多種錯拼 |
