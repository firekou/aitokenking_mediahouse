---
name: reel-dw-fmvme9-i
description: （草稿）我发现很多人提取风格 來源 IG Reel DW_fmVmE9_i。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
metadata:
  ...
  mediahouse-source-shortcode: "DW_fmVmE9_i"
...
>
> **來源：** [https://www.instagram.com/p/DW_fmVmE9_i/](https://www.instagram.com/p/DW_fmVmE9_i/)｜2026-04-11｜
> 89 秒｜195 views
> **分流：** 動作分 13（門檻 3）｜訊號 ['步驟序數（第N步）', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 我发现很多人提取风格,就是把图丢给AI,让它反推提取词。

## 疑似步驟（規則萃取，未經整理）

1. 这样提取到的根本不是风格,而是那次生成的结果。
2. 第一步,不要看画面有什么,而是看没什么。
3. 因为AI在做的其实是语义理合,它会用最安全的词语去概括结果,把这些词对生成几乎没有任何约束力。
4. 我们要做的是先找到画面的约束点,正确的问法应该是问AI这张图在生成时有哪些不可变的条件,是构图和视角,还是空间关系。
5. 第二步,让AI看共性而不是看独立。
6. 这些被反复保留下来的选择,才是真正的风格。
7. 第三步,复用画面结构搭建体系。
8. 如果你只停留在提取一张风格图,其实没有长期价值,真正有价值的是把前两步得到的条件,翻译成AI能长期执行的生成结构。
9. 所以,风格从来不是一批线词,而是一套被反复执行的选择逻辑。
