---
name: reel-dcpxnbldb1q
description: （草稿）教大家如何用AI在一分钟内生成这种品牌级的画面 來源 IG Reel DcPxNbLDB1Q。 逐字稿提到的工具（⚠️ 未經畫面確認）：Banana。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 教大家如何用AI在一分钟内生成这种品牌级的画面 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DcPxNbLDB1Q/](https://www.instagram.com/p/DcPxNbLDB1Q/)｜2026-08-20｜
> 46 秒｜1138 views
...
> 今天教大家如何用AI在一分钟内生成这种品牌级的画面，真的超简单的。

## 疑似步驟（規則萃取，未經整理）

1. 今天教大家如何用AI在一分钟内生成这种品牌级的画面，真的超简单的。
2. 首先需要拍几张产品的照片，正好是多个角度的。
3. 然后我们打开RTX注定有上传刚拍的图片作为参考，选择Nado Banana，然后输入你想要的画面描述。
4. 接着你可以调整角度，改变环境，调光线，修细节。
5. 如果要这种微距镜头效果，只需要截图你想放大的区域，再上传为新的参考图就好。
6. 如果想要看起来更专业一点，可以找一些产品的结构图或拆解图作为参考一起上传。
7. 最后我们可以通过剪印拼接画面，就能得到最终效果了。

## 逐字稿提到的工具

| 名稱 | 證據強度 | 說明 |
|---|---|---|
| `Banana` | ⚠️ **E5** | 逐字稿聽寫，**未經畫面確認**。實測顯示同一工具可能有多種錯拼 |
