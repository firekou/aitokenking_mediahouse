---
name: product-into-classical-painting
description: 三步工具鏈。解決：想把產品自然融進古畫風格的畫面裡並讓它動起來。 當使用者問到這類問題、或提到「產品融入古畫」時使用此 skill。⚠️ 技巧內容為影片作者自述（E6），我方未實測，不得作為效果承諾依據。
license: MIT
...
metadata:
  ...
  mediahouse-source-shortcode: "DbVRrxuDReO"
...
> **來源：** [https://www.instagram.com/p/DbVRrxuDReO/](https://www.instagram.com/p/DbVRrxuDReO/)｜刺蝟星球｜2026-07-28｜43 秒
> **證據強度：** 技巧效果 **⛔ E6（作者自述，我方零實測）**／步驟與坑 **E1（逐字稿人工重讀）**／
> 工具名 **⚠️ E5（聽寫，未經畫面確認）**
> **語言：** 一律繁體中文輸出。

**母題：** 文生圖做古畫底 → 產品替換 → 生成動畫。三個工具各做一件事。
...
**你的問題是不是這一個：想把產品自然融進古畫風格的畫面裡並讓它動起來。**
...
1. **用文生圖模型生成古畫內容。提示詞以**主體 ＋ 場景 ＋ 風格**為主，並明確畫面的顏色與構圖**
   → 產出：古畫底圖（多生成幾張不同類型）
2. **換到另一個模型把自己的產品融入畫卷——提示詞很簡單，告訴它替換產品就好**
   → 產出：含產品的古畫
3. **交給影片模型生成動畫**
   → 產出：成片
...
**坑 1｜**★ **要多生成幾張不同類型的畫面再挑**，不要第一張就往下做。

> 證據：作者原話

**坑 2｜**⚠️ **本片內容單薄**：全片 51 秒，只給了三步工具鏈，**沒有講任何失敗情況、沒有講邊界**。這支的價值是流程順序，不是深度。
...
這是一條特定風格的工具鏈。換風格就要重找模型與提示詞，本流程不遷移。

---

## 核心

三個模型各做一件事：**生底圖、換產品、讓它動**——不要指望一個模型全包。
...
| 聽寫 | 可能是 | 用途 | 強度 |
|---|---|---|---|
| `齐梦 3.1` | 即夢 3.1（推測） | 文生圖 | ⚠️ **E5** |
| `Nano Banana` | NanoBanana | 產品替換 | ⚠️ **E5** |
| `可铃` | 可靈（推測） | 生成動畫 | ⚠️ **E5** |
