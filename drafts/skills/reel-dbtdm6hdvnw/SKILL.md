---
name: reel-dbtdm6hdvnw
description: （草稿）我发现了一个AI谐星玩法 來源 IG Reel DbTDm6hDVNw。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DbTDm6hDVNw/](https://www.instagram.com/p/DbTDm6hDVNw/)｜2026-07-27｜
> 107 秒｜40665 views
> **分流：** 動作分 9（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 最近我发现了一个AI谐星玩法,它不仅能保留参考图的构图和机位,还不会被原图的画风影响,甚至可以精准复刻一整段视频的动作和运镜。

## 疑似步驟（規則萃取，未經整理）

1. 首先你要知道像这样的参考图,你要看到的不只有构图,人物背景还有材质画风,它都会一起参考,所以有时候你明明只想参考一个机位,生成结果却越来越像原图,这就是它的原因。
2. 首先第一步,把你喜欢的参考图上传给AI,再粘贴我这段绘模提示词,让它保持原图的结构完全不变。
3. 得到绘模以后,我们就可以用这张图去给Banana做参考生成画面,和直接给参考的相比,绘模生成的会更精准,并且不会被原图的色彩所干扰。
4. 那这里有一个注意点,就是千万不要直接告诉AI要生成绘模,这样它很可能会重新设计构图。
5. 如果你学会了这个,那聪明的你肯定想到了这个方法完全可以用到视频生成力,比如复杂动作参考,还有人物走位和镜头轨迹的控制都可以用同样的思路来实现。
6. 我们只需要把参考视频拖进CallDice,再粘贴我这段处理指令,它就会把原视频转换成黑白深度视频,接着我们再把这个素材和你的资产一起上传给Silence,它就能一比一复刻这些复杂的困难镜头了。
