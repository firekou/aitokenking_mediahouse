---
name: reel-dxe2hkoe40n
description: （草稿）我发现90%的人搭工作流的第一步就是错的 來源 IG Reel DXE2HKOE40n。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我发现90%的人搭工作流的第一步就是错的 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DXE2HKOE40n/](https://www.instagram.com/p/DXE2HKOE40n/)｜2026-04-13｜
> 80 秒｜166 views
> **分流：** 動作分 10（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞']
> **語言：** 一律繁體中文輸出。
...
> 我发现90%的人搭工作流的第一步就是错的,很多人上来的第一步永远是先写提示词,结果流程越搭越长,效果却一次比一次不稳定,今天三个步骤教你正确的工作流搭建方法。

## 疑似步驟（規則萃取，未經整理）

1. 我发现90%的人搭工作流的第一步就是错的,很多人上来的第一步永远是先写提示词,结果流程越搭越长,效果却一次比一次不稳定,今天三个步骤教你正确的工作流搭建方法。
2. 第一步也是最关键的一步,理清工作思路,思路是搭建工作流中最重要的环节,它决定了后续的节点安排,也决定了整个流程是否高效。
3. 第二步才轮到提示词,这里的提示词不是让你一句一句手写,真正能用的工作流里,提示词是结构化的,是可以被复用调整的,你可以让Gemini帮你跑,然后用Json去约束,让它按规则出结果,提示词在工作流里只是一个模块,并不是整体的核心。
4. 第三步,测试结果反馈错误点,结果不对千万别第一时间改提示词,先反推是哪一步出了问题,是目标没定清还是结构本身有问题,真正会搭工作流的人不是靠运气而是靠复盘。
