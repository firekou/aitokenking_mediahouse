...
| **逐字稿品質好不好** | **⛔ E6** | **我方零實測**。Apify 未揭露它用哪套 ASR，中文技術詞的誤字率未知 |
| 一次能抓滿幾支 | ⬜ **未量測** | `resultsLimit` 上限與實際回傳量的落差未驗 |

**⚠️ 第四列是這支 skill 最該被質疑的地方。**
`video-ingest` 對自家 ASR 明列了誤字表（Control Night＝ControlNet、口袋＝Codex…），
**Apify 的逐字稿我方連一次都沒對過**。第一批抓回來時，
**先人工回看兩三支**再決定要不要信它——`asr_reviewed` 欄位預設 0 就是為了這件事。
