---
name: reel-daqc-x7fepe
description: （草稿）看到的这些AI画面 來源 IG Reel DaQC_x7FePE。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DaQC_x7FePE/](https://www.instagram.com/p/DaQC_x7FePE/)｜2026-07-01｜
> 131 秒｜9059 views
...
> 你现在看到的这些AI画面,全是我靠资产、家庭式词生成的,中间没用一张参考图。

## 疑似步驟（規則萃取，未經整理）

1. 你现在看到的这些AI画面,全是我靠资产、家庭式词生成的,中间没用一张参考图。
2. 首先AI资产有三个核心大类,分别是角色场景、道具和色板。
3. 我们先从人物资产开始,首先一个完整的人物资产至少要包含这三类内容。
4. 场景其实有一种新的方法教给大家,就是九宫格法,前八张是你场景各个角度的画面,最后一张是平面布局图,这样AI在生成的时候就会参考这个布局来规划,整体的一致性和细节都能有比较好的还原。
5. 很多人做色板就是随便放几块颜色,但这样其实没什么用,真正有用的色板资产要包含三层信息,第一层是颜色本身,第二层是这个颜色的视觉属性,第三层是它的使用场景,这才是色板正确的生成方式。
6. 最后再讲两个最容易踩的坑,第一,资产不要做太少,一张人物图就想让AI生成整条片子,这肯定是不可能的,第二个,资产不要太花,你在资产阶段加入太多氛围,AI反而生成的不稳定。
