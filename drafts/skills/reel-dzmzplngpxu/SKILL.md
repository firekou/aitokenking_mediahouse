---
name: reel-dzmzplngpxu
description: （草稿）看到的这些AI画面 來源 IG Reel DZmzPLngPXu。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
metadata:
  ...
  mediahouse-source-shortcode: "DZmzPLngPXu"
...
>
> **來源：** [https://www.instagram.com/p/DZmzPLngPXu/](https://www.instagram.com/p/DZmzPLngPXu/)｜2026-06-15｜
> 106 秒｜6145 views
...
> 你现在看到的这些AI画面,我全程没写一句提示词,都是靠这些草图生成的。

## 疑似步驟（規則萃取，未經整理）

1. 你现在看到的这些AI画面,我全程没写一句提示词,都是靠这些草图生成的。
2. 首先你要知道,提示词能体现有三个种类,文本只是其中之一。
3. 在生图和生视频的时候,图片提示词的权重是最高的,所以你只要给足图片里的信息,哪怕你画得很简陋,AI也能给你生成出来。
4. 首先第一步肯定是先把你脑海里的画面画出来,这里完全不需要绘画基础,能用火柴人代替就可以,场景也只需要画出大致轮廓。
5. 但是,有了这手绘画面只是第一步,如果你直接拿这张图去生成,AI完全不懂你要的是什么风格,所以我们还需要再准备两个画面,一个是人物的四视图,另外一个是场景图。
6. 之后我们把这几张图片全部上传到AI画布里,然后粘贴我的这段提示词,把括号里的内容改成你脑子里的大致画面,这样AI就能复刻了。
7. 这里有一个点要注意,就是如果要做出我这种效果的话,一定要在相机这里选择和我一样的设置。
8. 当然这个方法肯定会增加抽卡的概率,大家可以先像我一样生成四宫格的画面,然后单独提取满意的高清放大,这样比较节省token。
