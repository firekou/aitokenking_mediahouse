---
name: reel-dzlrdn2zupu
description: （草稿）看到的这些恋爱画面 來源 IG Reel DZlrDN2zUpu。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DZlrDN2zUpu/](https://www.instagram.com/p/DZlrDN2zUpu/)｜2026-06-15｜
> 127 秒｜18380 views
> **分流：** 動作分 6（門檻 3）｜訊號 ['序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 你现在看到的这些恋爱画面,我全程没写一句提示词,全是CodeX帮我自动生成的。

## 疑似步驟（規則萃取，未經整理）

1. 你现在看到的这些恋爱画面,我全程没写一句提示词,全是CodeX帮我自动生成的。
2. 首先第一个,用CodeX做AI短剧。
3. 我们只需要把这两个Skill丢给CodeX,然后再告诉我AI你想要的大致风格,它就会自动调用这两个Skill,从剧本人物到分镜,按照固定流程一步步执行,然后再粘贴我这段提示词,它就会操控你的电脑在AI画布分别上传资产和提示词。
4. 整个过程你不需要逐条查看素材,只需要检查最后的效果。
5. 小到贪吃蛇,大到二维像素游戏都能做出来,我们只需要把这段提示词粘贴给CodeX,它就会自动调用印力扎尔和GPT5.5来打造游戏世界,包括角色设定、任务模式还有世界观,它都能自动给你生成,你只需要不断试玩,然后告诉它哪里不满意,让它修改就可以了。
