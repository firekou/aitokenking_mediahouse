---
name: reel-dy-gv1rej7z
description: （草稿）我用了一百多条提示词总结出了我们超实用的GPT应用叫玩法 來源 IG Reel DY_gv1REj7z。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我用了一百多条提示词总结出了我们超实用的GPT应用叫玩法 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DY_gv1REj7z/](https://www.instagram.com/p/DY_gv1REj7z/)｜2026-05-31｜
> 125 秒｜1499 views
> **分流：** 動作分 4（門檻 3）｜訊號 ['序列詞', '先…再']
> **語言：** 一律繁體中文輸出。
...
> 我用了一百多条提示词总结出了我们超实用的GPT应用叫玩法。

## 疑似步驟（規則萃取，未經整理）

1. 在视频开始前要先声明一点,因为GPT2的效果生成的非常真实,所以大家千万不要因为好奇心去违反法律法规。
2. 比如这个短剧片段,就是我用这套方法直接生成的。
3. 但现在你可以先让英明车尔做角色设定,让他给你生成角色,道具,甚至是世界观氛围,然后再让他根据剧情直接帮你生成分镜。
4. 不仅有画面参考,还有景别,运镜,然后你再丢给视频模型直接生成成片。
5. 玩法二,品牌物料生成,这个也非常夸张,以前用AI做图最难的一点就是文字,尤其是像这种电商详情页。
6. 但是现在你只需要把产品图片给他,AI就能把文字也生成得非常完美。
7. 另外你还可以用它做海报设计以及游戏场景设计,因为他有非常大的知识储备,所以你并不需要很详细的提示词也能生成。
8. 玩法三,AI产品原型,这是目前来讲比较火的隐藏玩法,我们可以先用GPT生成景箱UI设计图,再用Cousin去按照逻辑写代码,像产品原型,小游戏或者网页Demo都可以写。
9. 但是这个技巧还是需要一定的门槛,所以大家可以先等等,让AI再进化一段时间。
