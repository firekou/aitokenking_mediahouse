---
name: reel-dyo3goxetbt
description: （草稿）这些画面之所以不像AI 來源 IG Reel DYo3gOxEtBT。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DYo3gOxEtBT/](https://www.instagram.com/p/DYo3gOxEtBT/)｜2026-05-22｜
> 90 秒｜2249 views
> **分流：** 動作分 15（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞', '先…再', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 这些画面之所以不像AI,是因为从一开始我就没用提示词生成。

## 疑似步驟（規則萃取，未經整理）

1. 这些画面之所以不像AI,是因为从一开始我就没用提示词生成。
2. 第一步,先研究你想要的美学风格,我们可以选一部你喜欢的电影作为参考,然后去拆解它的拍摄条件来复刻它的画面。
3. 比如拍摄的相机是哪些,使用的镜头又是哪些,接着确定画幅比例,不同的画面大小会给人带来不同的感觉。
4. 最后是色彩,色彩决定了氛围,是画面的底色,我们可以参考各种相机的预设风格来匹配画面,比如科达的胶片风或者富士相机的冷美感。
5. 第二步,写一套JSON风格的提示词,我们要根据之前的研究在代码结构里锁定构图,灯光,渲染质量,同时加入硬因素,比如不允许数码锐化,或者不能生成文字,来确保画面是纯粹的电影级真实感。
6. 第三步,让画面动起来。
7. 动员画面已经足够干净,所以生成的运动会更自然。
8. 但前提是你必须得说对话,比如要先做环绕,再做推进,最后简直在一起,电影级的画面就生成了。
