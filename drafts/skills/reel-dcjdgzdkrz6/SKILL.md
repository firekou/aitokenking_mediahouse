---
name: reel-dcjdgzdkrz6
description: （草稿）想要做出AI活人感 來源 IG Reel DcJDGZDkrZ6。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DcJDGZDkrZ6/](https://www.instagram.com/p/DcJDGZDkrZ6/)｜2026-08-17｜
> 101 秒｜9047 views
> **分流：** 動作分 7（門檻 3）｜訊號 ['序列詞', '先…再', '操作動詞']
> **語言：** 一律繁體中文輸出。
...
1. 首先第一个也是最重要的就是人物的脸。
2. 很多人在上传素材的时候会只上传一张人物的四肢图,但其实这样是不够的,因为参考图只能告诉AI这个人物大概长什么样,却没有告诉他哪些特征必须不变。
3. 虽然C段子2.5对人物素材的理解和一致性提升了很多,但经过我多轮测试,如果想要让人物更真实,我们还是要在提示词里去做一些限定,首先给每个人物设计一个固定编号,接着从参考图里提取三到五个辨识度最高的面部特征,最后写成这种格式,这样人物会更精准真实一点。
4. 很多人会用AI生成的图片去做场景素材,但是这样会有一种被合成进场景的感觉。
5. 我们可以上网找一些真实的拍摄场景,但这里有个注意点,就是最好找iPhone拍摄的环境,不要去找摄影机的照片,因为摄影机照片的曝光、谨慎和构图都太精致,生成出来反而不像真实生活。
6. 最后一个是道具,道具的作用不只是丰富画面,更重要的是给人物找事情做。
7. 最后写完整提示词的时候,不要把所有内容塞进一段画里,按照参考图例的方法,先写画面质感与拍摄参数,再写人物身份,接着写场景道具,最后再用时间轴安排每个镜头,这样你的画面在生成出来之后就会很真实。
