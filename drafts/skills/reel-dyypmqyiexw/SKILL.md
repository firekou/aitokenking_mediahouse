---
name: reel-dyypmqyiexw
description: （草稿）我刚刚发现了一个AI新玩法 來源 IG Reel DYypmqYiEXw。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DYypmqYiEXw/](https://www.instagram.com/p/DYypmqYiEXw/)｜2026-05-26｜
> 66 秒｜958 views
...
> 我刚刚发现了一个AI新玩法,我叫它AGX调色法,它可以不用任何提示词自动给AI图调色。

## 疑似步驟（規則萃取，未經整理）

1. 首先你需要上传几张参考图到工具里,这个模型就会自动扫描画面,并提取里面的AGX色值,然后生成一套专属的调色版。
2. 你只要在生成的时候选择这套AGX色版,然后输入任何提示词,生成的图片都会自动带有相同的标志性色彩。
3. 这个方法的本质是把画面的颜色控制,从后期调色提前到了生成之前。
4. 所以这些画面并不是生成完再调出来的,而是在生成的时候颜色就被提前锁定了。
5. 它之所以看起来这么统一,是因为它本来就不是随机生成的。
