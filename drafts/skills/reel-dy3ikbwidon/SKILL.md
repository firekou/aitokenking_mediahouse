---
name: reel-dy3ikbwidon
description: （草稿）如果你用过AI 來源 IG Reel DY3IkbwiDon。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DY3IkbwiDon/](https://www.instagram.com/p/DY3IkbwiDon/)｜2026-05-28｜
> 90 秒｜1022 views
...
> 如果你用过AI，那你肯定遇到过这个问题，就是很难保持产品在AI视频生成时的一致性。

## 疑似步驟（規則萃取，未經整理）

1. 如果你用过AI，那你肯定遇到过这个问题，就是很难保持产品在AI视频生成时的一致性。
2. 问题就出在这里,如果你给的是模糊信息,比如只给了提示词,或者只给了一张要替换的产品图,那这些东西信息太少了,就算你用最新的AI模型,它生成的每一帧依然会有变化。
3. 比如这个东西,它的形状更接近圆形,那我们可以先生成一组这样的视频,然后先用这组图去生成视频,接着再把这段视频每隔一秒导出一张竞争画面,接着用banana把你的产品替换上去,最后再把这些替换后的画面重新丢给可琳Omini参考,这样你就能得到产品非常稳定的画面了。
4. 第二个方法其实和我们上期讲的是一样的,就是直接用banana生成一张替换后的图,再用这张图和之前的视频一起,给可琳用mini让它修改,但是这种方法初看可以,如果你要商用或者应对甲方可能还是不够的。
