---
name: reel-dch4l4tel9p
description: （草稿）教大家如何用AI制作宠物品类的产品宣传视频 來源 IG Reel Dch4L4tEl9p。 逐字稿提到的工具（⚠️ 未經畫面確認）：Studio。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 教大家如何用AI制作宠物品类的产品宣传视频 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/Dch4L4tEl9p/](https://www.instagram.com/p/Dch4L4tEl9p/)｜2026-08-27｜
> 51 秒｜1006 views
> **分流：** 動作分 15（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 今天教大家如何用AI制作宠物品类的产品宣传视频,真的超简单的。

## 疑似步驟（規則萃取，未經整理）

1. 首先我们打开OTX Studio,我们主要专注三个关键点,元素、构图和彩彩,然后用提示词来让画面保持一致性就好。
2. 第一步我们要从产品和主体开始,点击图像生成,然后在这里上传我们的产品和模特。
3. 接着我们学好比例模型之后,直接告诉AI我们要怎么结合。
4. 第二步我们要确定色彩方案,你可以在提示词里直接描述色彩,也可以上传色板,让系统跟阵走。
5. 接下来把所有画面加入Storyboard,看看整套广告放在一起的感觉,这样就能继续创造更多场景。
6. 你也可以把生成的图片导入InJourney来生成动态视频,提示词和学习方法我都整理好了,有需要的话可以找我拿,带来换去试试吧。

## 逐字稿提到的工具

| 名稱 | 證據強度 | 說明 |
|---|---|---|
| `Studio` | ⚠️ **E5** | 逐字稿聽寫，**未經畫面確認**。實測顯示同一工具可能有多種錯拼 |
