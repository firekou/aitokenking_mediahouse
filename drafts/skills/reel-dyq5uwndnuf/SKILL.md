---
name: reel-dyq5uwndnuf
description: （草稿）我发现很多人反推提示词只会把图丢给AI 來源 IG Reel DYq5uWnDNuf。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我发现很多人反推提示词只会把图丢给AI — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYq5uWnDNuf/](https://www.instagram.com/p/DYq5uWnDNuf/)｜2026-05-23｜
> 89 秒｜457 views
> **分流：** 動作分 3（門檻 3）｜訊號 ['先…再', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 我发现很多人反推提示词只会把图丢给AI,其实这本质和抽卡没有任何区别。

## 疑似步驟（規則萃取，未經整理）

1. AI会把结果当成原因,从而忽略画面生成背后的关键条件。
2. 方法二,生成结构骨架。
3. 真正贯穿整个生成过程的是构图,是光照方向,是主体与背景之间的关系。
4. 正确的做法是让AI从生成逻辑出发,反推画面背后的结构骨架,否则只是在不断放大随机性。
5. 我们应该让AI参与到纠错这一步,把反推得到的提示词先生成一张画面,再让AI对比画面,进行自我分析。
6. 正确利用AI反推提示词并不是让AI再描述一遍画面,而是让它站在生成系统的角度回答问题。
