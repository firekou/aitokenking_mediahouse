---
name: reel-dyjld2jjuhx
description: （草稿）AI提示词里有一个特殊的词叫扰动词 來源 IG Reel DYjLd2jjUhX。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# AI提示词里有一个特殊的词叫扰动词 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYjLd2jjUhX/](https://www.instagram.com/p/DYjLd2jjUhX/)｜2026-05-20｜
> 77 秒｜455 views
...
> AI提示词里有一个特殊的词叫扰动词,它不描述画面内容,而是作为干扰信号存在,让AI在生成画面时优先处理它。

## 疑似步驟（規則萃取，未經整理）

1. AI提示词里有一个特殊的词叫扰动词,它不描述画面内容,而是作为干扰信号存在,让AI在生成画面时优先处理它。
2. 我们可以在抽象词前加扰动词,先分流注意力,然后把抽象词拆成可量化视觉动作,AI注意力被打散后,主体就不会被夸张效果覆盖。
3. 比如你写森林里的女孩,AI会自动补光,补舞,补背景,补到最后主体可能走样。
4. 加入扰动词后,AI会先处理扰动词,再画主体,这样的内容会更有风格,主体也会更清晰。
