---
name: reel-dbcngkomfhc
description: （草稿）教大家如何在30秒内快速创建一个高效的AI分镜脚本 來源 IG Reel DbCnGKOmfhc。 逐字稿提到的工具（⚠️ 未經畫面確認）：Studio。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 教大家如何在30秒内快速创建一个高效的AI分镜脚本 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DbCnGKOmfhc/](https://www.instagram.com/p/DbCnGKOmfhc/)｜2026-07-21｜
> 48 秒｜1015 views
> **分流：** 動作分 7（門檻 3）｜訊號 ['序列詞', '操作動詞', '具體參數']
> **語言：** 一律繁體中文輸出。
...
> 今天教大家如何在30秒内快速创建一个高效的AI分镜脚本。

## 疑似步驟（規則萃取，未經整理）

1. 首先进入LTX Studio,我们先建一个故事板,然后选择一个空白分镜,接着我们选择风格,点击故事板之后输入一些基础提示词来描述你希望在广告画面中发生的内容。
2. 当然用英文会生成的更加准确一些,所以我们翻译一下就好了。
3. 当图像生成后,点击这个Add Image导出图片,然后点一下左上角的一直为按钮系统会自动把当前图像加入提示栏中。
4. 接下来,你可以上传产品实拍参考图,并选择Nano Banana作为模型,系统就会自动融合这两张参考图。
5. 这样,你就能为客户生成一个完整的AI分镜脚本。
