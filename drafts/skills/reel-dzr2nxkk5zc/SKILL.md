---
name: reel-dzr2nxkk5zc
description: （草稿）我用AI做的打斗画面 來源 IG Reel DZr2nXKk5zC。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/DZr2nXKk5zC/](https://www.instagram.com/p/DZr2nXKk5zC/)｜2026-06-17｜
> 103 秒｜4605 views
> **分流：** 動作分 11（門檻 3）｜訊號 ['步驟序數（第N步）', '序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
> 这是我用AI做的打斗画面,不仅动作镜头连贯,而且有力量感。

## 疑似步驟（規則萃取，未經整理）

1. 首先,很多人做不出打击感的原因,是因为你和AI都不知道这招到底是什么。
2. 所以第一步一定要先把动作的名称告诉AI。
3. 我整理的这些图片里有大部分的动作技巧名称,然后你再让AI根据这些动作帮你写一套打斗提示词,效果会比之前的好很多。
4. 如果你什么都不会写,那你只要记住一个关键词就够了,那就是X视角,SAT就是动作游戏视角,它一共有三个作用,一个是它能让镜头跟着攻击方向走,第二个是它能让命中瞬间有停顿感,画面会更有重量,第三个它能自动强化镜头反馈,比如轻微镜头震动,画面受到打击的反馈,这些细节能快速让观众入戏,但让我这里还有三种更高级的运镜提示词,分别是跟随主角动量运镜,跟随敌人受力反馈运镜,连续一镜到底运镜,提示词模板大家可以截图保存,但我测试下来,如果想让AI打戏真正稳定,其实还是要用到故事版加提示词一起控制,我们在让AI生成提示词后,可以用一命这样的生成对应的故事版,然后再把提示词和故事版一起给AI,生成的画面是最稳定的,我把我这个打动的技能同样做成了skill,你只要把角色和场景一起上传给agent,它就能自动分析角色的性格特征,给你一段打动的提示词,同样我也放在资料里了,所以这个方法你学会了吗?
