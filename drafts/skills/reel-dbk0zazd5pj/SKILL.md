---
name: reel-dbk0zazd5pj
description: （草稿）我发现AI圈其实是个轮回 來源 IG Reel Dbk0zAzD5Pj。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
>
> **來源：** [https://www.instagram.com/p/Dbk0zAzD5Pj/](https://www.instagram.com/p/Dbk0zAzD5Pj/)｜2026-08-03｜
> 110 秒｜15523 views
...
> 我发现AI圈其实是个轮回，因为早就过时的方法放进现在的新模型里反而成为了最好用的。

## 疑似步驟（規則萃取，未經整理）

1. 首先第一个深度图,上一期我们讲过深度图可以一比一迁移人物动作,但其实它还有一个进阶的玩法,就是可以用它把一张平面图片变成一个可以重新选择机位的空间。
2. 首先我们用提示词提示完深度图后,把这张图丢给抠袋,让它根据深度关系把画面构建成一个可以移动机位的三维场景。
3. 这时候移动虚拟摄像机,就能重新拍摄这个场景,然后把这些的新视角和资产一起交给AI,这样就能生成不同角度,但是动作还原的画面了,你甚至还可以修改主体的比例,做出巨物压迫,微缩世界这样的画面。
4. 第二个就是法线图,法线图在建模的时候会经常用到,但其实我们完全可以用它来辅助我们制作人物角色,我们可以把原图交给AI,让它提取人物的法线图,再让AI按照法线图重新生成人物角色的纹理,这样重新生成的人物,不只是皮肤变得更清晰,五官衣服褶皱和首饰雕刻都会更有感觉,甚至可以在保持面部结构不变的情况下,重新更换妆容。
5. 第三个是动态轮廓图,它能解决的是AI产品一致性的问题,比如我们想让一个logo变成产品,直接写进提示词,AI通常只会做一个简单的溶解效果,但我们可以把logo和产品交给Codec,让它分别提取两者的轮廓,再自动生成中间不同阶段的轮廓变化,最后交给视频模型补充材质和特效,这样AI就能在生成的过程中保持产品的一致性。
