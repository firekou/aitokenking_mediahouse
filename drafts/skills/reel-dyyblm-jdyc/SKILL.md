---
name: reel-dyyblm-jdyc
description: （草稿）我发现AI视频里最容易露馅的其实不是画面 來源 IG Reel DYyBLm-jDYC。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我发现AI视频里最容易露馅的其实不是画面 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYyBLm-jDYC/](https://www.instagram.com/p/DYyBLm-jDYC/)｜2026-05-26｜
> 95 秒｜2009 views
...
> 我发现AI视频里最容易露馅的其实不是画面,而是声音,很多人的画面做的已经很真实了,但只要人物一开口,立刻就显得很假,今天三个方法教你解决这个棘手问题。

## 疑似步驟（規則萃取，未經整理）

1. 方法一,建立角色声音参考,很多人做AI视频的时候,每次都会让AI重新生成配音,这样只会增加AI配音的不稳定性,正确的做法是给每个角色建立声音舞台,如果你生成的片段里有合适的音色,就把这段视频下载下来,再把音频保存成一个独立的音频文件,之后每次生成视频的时候,就可以把这段音频上传作为声音参考,这样声音的一致性会稳定很多,当然你也可以从经典电影中提取或者是自己录一段上传,不过要注意版权的问题。
2. 方法二,用独立配音模型设计角色声音,那还有一种做法就是把配音独立出来,这里有一个比较专业的做法,首先先把角色图片和人物性格设定交给Gemini,让它来帮你设计角色音色,提示词可以参考我这个,然后再用Gemini给的需求生成配音,选到满意的声音以后就可以长期使用这个音色了,我把这些AI技巧都传到站点里了,如果有需要的话可以找我拿。
3. 方法三,建立角色声音库,同一个角色在不同情境下,声音的语气、气息、节奏都会发生变化,我们可以刻意生成一组情绪样本,比如正常说话、愤怒、伤心,把每一种情绪都生成一小段语音保存,这样你就会得到同一个角色在不同情绪状态下声音的标准,以后生成新的对白,只要根据剧情情绪选择对应的声音参考,角色不仅音色统一,连情绪表现也会非常稳定,AI创作真正难的不是人物一致性,而是如何让同一个角色持续存在,而声音就是这个角色存在的证据,关注我,带你学习更多AI思维技巧。
