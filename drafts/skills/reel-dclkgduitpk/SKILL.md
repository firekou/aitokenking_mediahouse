---
name: reel-dclkgduitpk
description: （草稿）教大家如何用AI做出超酷的产品转场效果 來源 IG Reel DcLkGdUiTPK。 逐字稿提到的工具（⚠️ 未經畫面確認）：Studio。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 教大家如何用AI做出超酷的产品转场效果 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DcLkGdUiTPK/](https://www.instagram.com/p/DcLkGdUiTPK/)｜2026-08-18｜
> 43 秒｜1626 views
...
> 今天教大家如何用AI做出超酷的产品转场效果，真的超简单的。

## 疑似步驟（規則萃取，未經整理）

1. 首先我们先拍摄一张想展示的产品照片，然后打开LTEX Studio上传照片，并选择Nano by Nano模型，接着我们让它变成一张精美的电商图。
2. 选择一张我们满意的图片导入ChatGPT，让GPT给我们生成多角度的产品拆解提示词。
3. 复制一份回到LTEX Studio，用刚才的生成的图和提示词来生成不同角度的产品镜头。
4. 最后一步，我们把这些图片全部做成完整的视频，这都在LTEX Studio里完成。
