---
name: reel-db-tvn3kgkn
description: （草稿）我用AI生成的十一人群像视频 來源 IG Reel Db-tvN3kgKn。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我用AI生成的十一人群像视频 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/Db-tvN3kgKn/](https://www.instagram.com/p/Db-tvN3kgKn/)｜2026-08-13｜
> 106 秒｜3251 views
> **分流：** 動作分 5（門檻 3）｜訊號 ['序列詞', '操作動詞', '教學祈使']
> **語言：** 一律繁體中文輸出。
...
1. 这是我用AI生成的十一人群像视频。
2. 首先很多人控制多人站位,第一时间想到的是导演台。
3. 比如我这里的紫色的模型,我上传的是绿色人物,但它却受到模型影响给了我紫色的。
4. 我们可以先生成一张场景俯视图,然后把整个空间划分成不同区域,再用数字给每个人物固定一个位置,接着把这张站位图交给Codec,让它把图中的位置转换成一份全局站位表。
5. 每个人需要记录这六项信息,这样它就会给你一段人物站位的文本提示词,接着把人物资产、编号站位图和全局站位表一起交给C4D,它就能保持多人站位一致性了。
6. 不过,这里有两个点要注意,第一个是画面第一幕一定要先用全景镜头,因为AI在连续生成的时候本身具有一定的上下文延续能力,如果第一幕人物的位置关系就已经建立准确了,后面的人物会准确很多,比如这一段,这个人物的出现就是因为我的第一幕里也有出现他。
7. 第二个是一定要把这段提示词写到前面,这样Codec生成的提示词才会保持多镜头的一致性。
