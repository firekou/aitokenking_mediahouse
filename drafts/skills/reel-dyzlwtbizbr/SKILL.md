---
name: reel-dyzlwtbizbr
description: （草稿）我刚刚发现了AI的一个写修玩法 來源 IG Reel DYzLWTBiZbr。 當使用者問到這支影片的做法、或想把它變成可執行流程時使用。⚠️ 這是自動產生的草稿，尚未經人工整理與實測，不得作為效果承諾依據。
license: MIT
...
# 我刚刚发现了AI的一个写修玩法 — ⚠️ 自動草稿

> **狀態：DRAFT。這不是一支可以直接用的 skill。**
> 它由規則從逐字稿萃取產生，**沒有經過人整理、沒有經過實測**。
> 晉升到 `skills/` 是人的動作，見檔尾《晉升前必須做的四件事》。
>
> **來源：** [https://www.instagram.com/p/DYzLWTBiZbr/](https://www.instagram.com/p/DYzLWTBiZbr/)｜2026-05-26｜
> 66 秒｜640 views
> **分流：** 動作分 4（門檻 3）｜訊號 ['序列詞', '操作動詞']
> **語言：** 一律繁體中文輸出。
...
> 我刚刚发现了AI的一个写修玩法,它可以不用任何提示词就能生成超真实的电影感效果,你现在看到的这些画面全部都是这个玩法生成的,但很多人不知道的是,这些电影感其实不是AI随机生成的,而是被一套规则控制着,这个玩法的本质其实很简单,就是把所有电影效果都做成了一套预设,也正因为这样,你不需要任何提示词也能生成电影感很强的内容,在生成的时候你可以直接选择电影摄影机的运镜方式,比如镜头跟随,镜头环绕,然后是视频的速度控制,这些都可以通过选项来选择,所以这些画面并不是通过提示词调出来的,而是在生成的时候就被提前约束了,也正因为这样每一次生成出来的画面都非常稳定,你不需要反复冲卡,也不用一遍一遍重写提示词,只需要选择镜头,画面就会按照电影的逻辑生成,我把这个斜秀玩法整理好了,如果你需要的话可以找我拿,很多人觉得AI创作就是拼提示词,但其实真正厉害的玩法是把复杂的东西变成简单的选择,当电影语言变成可以直接调用的工具,AI才会真正变成你的创作助手,关注我,我会继续分享更多这样的AI创作技巧。

## 疑似步驟（規則萃取，未經整理）

1. 我刚刚发现了AI的一个写修玩法,它可以不用任何提示词就能生成超真实的电影感效果,你现在看到的这些画面全部都是这个玩法生成的,但很多人不知道的是,这些电影感其实不是AI随机生成的,而是被一套规则控制着,这个玩法的本质其实很简单,就是把所有电影效果都做成了一套预设,也正因为这样,你不需要任何提示词也能生成电影感很强的内容,在生成的时候你可以直接选择电影摄影机的运镜方式,比如镜头跟随,镜头环绕,然后是视频的速度控制,这些都可以通过选项来选择,所以这些画面并不是通过提示词调出来的,而是在生成的时候就被提前约束了,也正因为这样每一次生成出来的画面都非常稳定,你不需要反复冲卡,也不用一遍一遍重写提示词,只需要选择镜头,画面就会按照电影的逻辑生成,我把这个斜秀玩法整理好了,如果你需要的话可以找我拿,很多人觉得AI创作就是拼提示词,但其实真正厉害的玩法是把复杂的东西变成简单的选择,当电影语言变成可以直接调用的工具,AI才会真正变成你的创作助手,关注我,我会继续分享更多这样的AI创作技巧。
